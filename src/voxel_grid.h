...
#ifndef SRC_VOXEL_GRID_H_
#define SRC_VOXEL_GRID_H_
...
class VoxelGrid {
 public:
  ...
  // Default constructor (creates an empty voxel grid object). This needs to be
  // initialized with LoadFromFile or BuildAroundPointCloud before using.
  VoxelGrid();

  // Creates a cubic VoxelGrid that contains a voxelized ball (sphere) of the
  // given radius. Voxels that are inside the sphere have a value of 1, and all
  // other voxels get a value of 0.
  ...
  // are found to be internal will receive a value of 1. All other voxels will
  // have a value of 0.
  ...
  // product between this grid and the given filter grid centered at (x, y, z).
  // Returns the sum of the dot product of the intersection.
  //
  ...
  // Returns the size string in the form of "W x H x D" where W is width, H is
  // height, and D is depth (x, y, z dimensions, respectively).
  ...
  // volume. The first line of the file contains the voxel size, minimum x, y,
  // z values of the grid, and the number of cells in the x, y, z axes. Each
  // subsequent line of the file represents a single voxel:
  //   x y z v
 ...
 private:
  ...
  //   grid_map_[x][y][z] = val;
  // Writing is possible to any arbitrary coordinate. Reading from an undefined
  // coordinate will automatically return a default value of 0.
  GridMap grid_map_;
